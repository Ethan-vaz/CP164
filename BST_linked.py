...
class BST:
    ...
    def __eq__(self, target):
        ...
        if self._count != target._count:
            ...
            equals = self.__eq_aux__(self._root, target._root)

        return equals
    
    def __eq_aux__(self, node1, node2):
        
        if node1 is None and node2 is None:
            equals = True
        elif node1 is not None and node2 is not None:
            if node1._value == node2._value and node1._height == node2._height:
                equals = self.__eq_aux__(node1._left, node2._left) and self.__eq_aux__(node1._right, node2._right)
            else:
                equals = False
        else:
            equals = False
    ...
    def parent_r(self, key):
        ...
        if key == node._value:
            found = True
            value = parent
        if parent is None and found is False:
            value = self.parent_r_aux(node, parent, found, key)

        
        return value

    def parent_r_aux(self, node, parent, found, key):
        
        if node is not None:
            if key < node._value:
                value = self.parent_r_aux(node._left, node, found, key)
            elif key > node._value:
                value = self.parent_r_aux(node._right, node, found, key)
            else:
                value = deepcopy(parent._value)
        else:
            value = None
    ...
    def max_r(self):
        ...
        current = self._root
        value = self.max_r_aux(current)
            
        return value
            
    def max_r_aux(self, current):
    ...
    def count(self):
        ...
        ----------------------------------------------------------
        """
        number = self.count_aux(self._root)
        
        return number

    def count_aux(self, current):
        
        if current is None:
            number = 0
        else:
            number = 1 + self.count_aux(current._left) + self.count_aux(current._right)      
